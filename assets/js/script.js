...
const filterFunc = function (selectedValue) {
  for (let i = 0; i < filterItems.length; i++) {
    ...
    if (selectedValue === "all" || selectedValue === category) {
      filterItems[i].style.display = "block";  // Tampilkan item
      filterItems[i].classList.add("active");
    } else {
      filterItems[i].style.display = "none";   // Sembunyikan item
...
document.getElementById("search-input").addEventListener("input", function () {
  ...
  projects.forEach(project => {
    ...
    if (title.includes(searchValue) || category.includes(searchValue)) {
      project.style.display = "block"; // Tampilkan proyek
    } else {
      project.style.display = "none"; // Sembunyikan proyek
    }
  });
});
...
document.getElementById("blog-search-input").addEventListener("input", function () {
  ...
  posts.forEach(post => {
    ...
    // Tampilkan artikel jika cocok dengan pencarian
    if (title.includes(searchValue) || category.includes(searchValue)) {
      post.style.display = "block"; // Tampilkan artikel
    } else {
      post.style.display = "none"; // Sembunyikan artikel
    }
  });
});
